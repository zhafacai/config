...
*** The =oxocarbon-theme.el= section
A real theme file so that `load-theme' (and thus theme-buffet) can find
oxocarbon.  Emacs binds themes to files: `modus-themes-theme' only works
once the theme's file is loaded, and `custom-theme-load-path' (added in
the themes section above) already points at lisp/ where this is tangled.
#+begin_src emacs-lisp :tangle lisp/oxocarbon-theme.el
...
#+end_src
*** The =oxocarbon-light-theme.el= section
Light counterpart of the dark oxocarbon theme, using the same Ef
machinery and the same IBM-Carbon accent identity (pink / teal / blue /
purple) on a light background.  A separate file is required because
`load-theme' finds themes by their `NAME-theme.el' file.
#+begin_src emacs-lisp :tangle lisp/oxocarbon-light-theme.el
...
(service home-bash-service-type
		 (home-bash-configuration
		   ...
		   (bash-profile (list (local-file
								...
								"bash_profile")))))
#+end_src
*** The =base.scm= section about openssh
~home-use~
