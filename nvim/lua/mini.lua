...
do
	local keys = {
		...
		{ action = ":Neogit", desc = "Git", icon = "\238\153\157 ", key = "n" },
		{ action = "<cmd>OrgSuperAgenda!<CR>", desc = "Agenda", icon = "\243\176\160\174 ", key = "a" },
		{ action = "<cmd>Org capture<CR>", desc = "Capture", icon = "\238\184\145 ", key = "c" },
		{ action = ":lua Snacks.dashboard.pick('live_grep')", desc = "Live Grep", icon = "\239\128\162 ", key = "f" },
		{ action = ":lua Snacks.dashboard.pick('oldfiles')", desc = "Recent Files", icon = "\239\131\133 ", key = "r" },
		{
		...
		{ desc = "Restore Session", icon = "\238\141\136 ", key = "s", action = ":lua MiniSessions.read()" },
		{ action = ":qa", desc = "Quit", icon = "\239\144\166 ", key = "q" },
	}
	local sections = {
		{ icon = "\239\132\156 ", indent = 2, padding = 1, section = "keys", title = "Keymaps" },
		{ icon = "\239\133\155 ", indent = 2, padding = 1, section = "recent_files", title = "Recent Files" },
		{ icon = "\239\129\188 ", indent = 2, padding = 1, section = "projects", title = "Projects" },
	}
	...
	require("snacks").setup({
		...
		dashboard = { enabled = true, preset = { keys = keys }, sections = sections },
		picker = { enabled = true },
	})
end
local function _1_()
