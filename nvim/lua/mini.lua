...
do
  local keys = {{action = ":lua Snacks.dashboard.pick('files')", desc = "Find File", icon = "\239\128\130 ", key = "<space><space>"}, {action = ":lua Snacks.dashboard.pick('live_grep')", desc = "Live Grep", icon = "\243\177\161\180 ", key = "f"}, {action = ":lua Snacks.dashboard.pick('oldfiles')", desc = "Recent Files", icon = "\239\131\133 ", key = "r"}, {action = ":Neogit", desc = "Git", icon = "\238\153\157 ", key = "g"}, {action = "<cmd>Org agenda<CR>", desc = "Agenda", icon = "\243\177\168\139 ", key = "a"}, {action = ":Denote", desc = "Denote", icon = "\238\185\180 ", key = "n"}, {action = "<cmd>Org capture<CR>", desc = "Capture", icon = "\243\176\155\168 ", key = "c"}, {action = ":lua Snacks.dashboard.pick('files', {cwd = '~/dots'})", desc = "Config", icon = "\239\144\163 ", key = "C"}, {desc = "Restore Session", icon = "\238\141\136 ", key = "s", action = ":lua MiniSessions.read()"}, {action = ":qa", desc = "Quit", icon = "\239\144\166 ", key = "q"}}
...
vim.keymap.set("n", "<leader>hd", _9_, {desc = "Diagnostics"})
local function _10_()
  return Snacks.picker.highlights()
end
vim.keymap.set("n", "<leader>hl", _10_, {desc = "Highlights"})
local function _11_()
  return Snacks.picker.keymaps()
end
vim.keymap.set("n", "<leader>hk", _11_, {desc = "Keymaps"})
local function _12_()
  return Snacks.picker.marks()
end
vim.keymap.set("n", "<leader>hm", _12_, {desc = "Marks"})
local function _13_()
  return Snacks.picker.colorschemes()
end
vim.keymap.set("n", "<leader>hs", _13_, {desc = "Colorschemes"})
local function _14_()
  return Snacks.gitbrowse()
end
vim.keymap.set("n", "<leader>gb", _14_, {desc = "Git browse"})
local function _15_()
  return Snacks.scratch()
end
vim.keymap.set("n", "<leader>ss", _15_, {desc = "Scratch buffer"})
local function _16_()
...
vim.keymap.set("n", "<leader>sI", _18_, {desc = "Icons"})
local function _19_()
  return Snacks.picker.gh_issue()
end
vim.keymap.set("n", "<leader>gi", _19_, {desc = "Issue"})
local function _20_()
  return Snacks.picker.gh_issue({state = "all"})
end
vim.keymap.set("n", "<leader>gI", _20_, {desc = "Issue All"})
local function _21_()
  return Snacks.picker.gh_pr()
end
vim.keymap.set("n", "<leader>gp", _21_, {desc = "Pr"})
local function _22_()
